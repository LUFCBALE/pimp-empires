...
def handle_socket_connect():
    ...
    db = get_db()
    user = db.execute('SELECT is_banned FROM users WHERE id = ?', (session['user_id'],)).fetchone()
    db.close()
...
def require_admin(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        db = get_db()
        user = db.execute('SELECT is_admin, is_banned FROM users WHERE id = ?', (session['user_id'],)).fetchone()
        db.close()
...
SEASON_END_PRIZES_ENABLED = True
...
def maybe_award_season_end_prizes(world):
    ...
    world['seasonEndAt'] = ge.now_ms() + ge.GAME_DURATION_MS
    world['seasonPrizesAwarded'] = False
    
    reset_world_state(world)
    save_world(world)


def reset_world_state(world):
    """Wipes all players back to default state (keeping only achievements and
    premium/meta progression), and clears bots from the world state."""
    db = get_db()
    
    # 1. Reset all human players, keeping meta progression
    rows = db.execute('''
        SELECT p.user_id, p.state_json, u.pimp_name
        FROM player_state p JOIN users u ON u.id = p.user_id
        WHERE p.user_id > 0
    ...
    for row in rows:
        try:
            old_s = json.loads(row['state_json'])
            new_s = ge.default_state(row['pimp_name'])
            
            # Retain meta progression
            new_s['achievements'] = old_s.get('achievements', [])
            new_s['mobDollars'] = old_s.get('mobDollars', 0)
            new_s['referredBy'] = old_s.get('referredBy', None)
            new_s['referralRewardPaid'] = old_s.get('referralRewardPaid', False)
            
            db.execute('''
                UPDATE player_state
                SET state_json = ?, updated_at = ?
                WHERE user_id = ?
            ''', (json.dumps(new_s), ge.now_ms(), row['user_id']))
        except Exception:
            pass

    # 2. Delete all bots from the world state (they will be regenerated by ensure_bots)
    world['bots'] = []
    world['botCrewEmblems'] = {}
...
def api_admin_settings_get():
    import re
    with open(os.path.join(BASE_DIR, 'game_engine.py'), 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Extract some key constants
    keys = ['TURN_REGEN_AMOUNT', 'TURN_REGEN_MS', 'STARTING_CASH', 'STARTING_TURNS', 'MAX_TURNS_BASE']
    settings = {}
    for k in keys:
        m = re.search(r'^' + k + r'\s*=\s*(.*?)\s*$', content, re.M)
        if m:
...
def api_admin_settings_post():
    ...
    filepath = os.path.join(BASE_DIR, 'game_engine.py')
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
        
    for k, v in data.items():
        if k in ['TURN_REGEN_AMOUNT', 'TURN_REGEN_MS', 'STARTING_CASH', 'STARTING_TURNS', 'MAX_TURNS_BASE']:
            # Replace the constant value
            content = re.sub(r'^' + k + r'\s*=.*$', f"{k} = {v}", content, flags=re.M)
