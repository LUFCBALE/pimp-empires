...
def reset_world_state():
    ...
    premium/meta progression), and deletes all bots (so they can respawn)."""
    db = get_db()
    ...
    # 2. Delete all bots (they will be regenerated by maybe_spawn_bots)
    db.execute('DELETE FROM player_state WHERE user_id < 0')
    db.commit()
    db.close()
